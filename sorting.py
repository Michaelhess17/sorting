...
def _merged(xs, ys, cmp=cmp_standard):
    ...
    "internal use only", and not part of the "public interface".
...
def quick_sort(xs, cmp=cmp_standard, lo=0, hi=None):
    ...
    merge step, and has a Theta(n) memory requirement.
    Even though quick sort and merge sort both have the same Theta(n log n)
    runtime, this more efficient memory usage typically makes quick sort
    faster in practice.
    ...
    #Stability), but this is typically inconsequential.
